...
export default function VoiceWidget({ apiKey, agentName, voice, wsUrl, mode, widgetBaseUrl }) {
  ...
  // ── WebSocket connect (with exponential-backoff reconnect) ───────
  useEffect(() => {
    ...
    const connect = () => {
      ...
      if (!apiKey) {
        setError('No API key — widget not configured correctly.');
        console.error('[widget] apiKey is empty, cannot authenticate.');
        return;
      }
