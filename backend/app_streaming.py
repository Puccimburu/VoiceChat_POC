...
def _emit_ordered(completed_results, next_to_emit, sid, request_id):
    ...
    while next_to_emit in completed_results:
        ...
        socketio.emit('audio_chunk', {'text': result['text'], 'audio': result['audio'], 'words': result['words'], 'request_id': request_id}, room=sid)
        next_to_emit += 1
    return next_to_emit
...
def handle_disconnect():
    ...
    if sid in streaming_sessions:
        try:
            ...
            logger.error(f"Error closing STT on disconnect: {e}")
...
def handle_end_speech(data):
    ...
    # Close STT and wait a tiny bit to avoid overlapping streams
    stt.close()
    time.sleep(0.05)  # small delay before starting new stream or allowing reuse

    # Log if the last stream had an error
    if last_stream_had_error():
        logger.warning(f"Previous streaming STT session for {sid} ended with an error; client may retry")
    ...
    logger.info(
        f"Streaming transcript: \"{transcript}\" (finalize: {stt_time:.3f}s, buffer: {buffer_size} bytes)"
    )
    ...
    try:
        ...
        with ThreadPoolExecutor(max_workers=3) as executor:
            ...
            for chunk in response_stream:
                if active_requests.get(sid) != request_id:
                    logger.info("Pipeline cancelled — no longer the active request")
                    break
